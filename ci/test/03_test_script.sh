...
if [[ "${RUN_IWYU}" == true ]]; then
  ...
  FILES_WITH_ENFORCED_IWYU="/src/(((crypto|index|kernel|primitives|univalue/(lib|test)|util|zmq|common)/.*|node/blockstorage|node/utxo_snapshot|clientversion|core_io|signet)\\.cpp)"
